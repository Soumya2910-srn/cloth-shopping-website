mens = {
    1: {
        'name': 'Casual Denim Jacket',
        'price': 999.00,
        'original_price': 1499.00,
        'image': 'denimjacket.jpg',
        'rating': 4.5,
        'reviews': 85,
        'category': 'topwear',
        'badge': '33% OFF'
    },
    2: {
        'name': 'Graphic T-Shirt',
        'price': 499.00,
        'original_price': 699.00,
        'image': 'graphictshirt.jpg',
        'rating': 4.7,
        'reviews': 48,
        'category': 'topwear',
        'badge': '29% OFF'
    },
    3: {
        'name': 'Checked Casual Shirt',
        'price': 699.00,
        'original_price': 999.00,
        'image': 'checkedshirt.jpg',
        'rating': 4.5,
        'reviews': 94,
        'category': 'topwear',
        'badge': '30% OFF'
    },
    4: {
        'name': 'Printed Hoodie',
        'price': 1199.00,
        'original_price': 1699.00,
        'image': 'hoodie.jpg',
        'rating': 4.6,
        'reviews': 67,
        'category': 'topwear',
        'badge': '29% OFF'
    },
    5: {
        'name': 'Mandarin Collar Shirt',
        'price': 899.00,
        'original_price': 1199.00,
        'image': 'mandarinshirt.jpg',
        'rating': 4.3,
        'reviews': 52,
        'category': 'topwear',
        'badge': '25% OFF'
    },
    6: {
        'name': 'Slim Fit Jeans',
        'price': 1299.00,
        'original_price': 1699.00,
        'image': 'slimfitjeans.jpg',
        'rating': 4.8,
        'reviews': 72,
        'category': 'bottomwear',
        'badge': '24% OFF'
    },
    7: {
        'name': 'Printed Lounge Pants',
        'price': 749.00,
        'original_price': 999.00,
        'image': 'loungepants.jpg',
        'rating': 4.4,
        'reviews': 41,
        'category': 'bottomwear',
        'badge': '25% OFF'
    },
    8: {
        'name': 'Cargo Trousers',
        'price': 1199.00,
        'original_price': 1599.00,
        'image': 'cargopants.jpg',
        'rating': 4.7,
        'reviews': 64,
        'category': 'bottomwear',
        'badge': '25% OFF'
    },
    9: {
        'name': 'Chino Pants',
        'price': 899.00,
        'original_price': 1299.00,
        'image': 'chinopants.jpg',
        'rating': 4.5,
        'reviews': 51,
        'category': 'bottomwear',
        'badge': '30% OFF'
    },
    10: {
        'name': 'Cotton Joggers',
        'price': 799.00,
        'original_price': 1099.00,
        'image': 'joggers.jpg',
        'rating': 4.6,
        'reviews': 46,
        'category': 'bottomwear',
        'badge': '27% OFF'
    },
    11: {
        'name': 'Cotton Kurta',
        'price': 799.00,
        'original_price': 1099.00,
        'image': 'cottonkurta.jpg',
        'rating': 4.6,
        'reviews': 65,
        'category': 'ethnic',
        'badge': '27% OFF'
    },
    12: {
        'name': 'Pathani Suit',
        'price': 1399.00,
        'original_price': 1899.00,
        'image': 'pathanisuit.jpg',
        'rating': 4.9,
        'reviews': 89,
        'category': 'ethnic',
        'badge': '26% OFF'
    },
    13: {
        'name': 'Embroidered Nehru Jacket',
        'price': 1099.00,
        'original_price': 1499.00,
        'image': 'nehrujacket.jpg',
        'rating': 4.8,
        'reviews': 59,
        'category': 'ethnic',
        'badge': '27% OFF'
    },
    14: {
        'name': 'Festive Kurta Pajama Set',
        'price': 1199.00,
        'original_price': 1699.00,
        'image': 'kurtapajama.jpg',
        'rating': 4.9,
        'reviews': 78,
        'category': 'ethnic',
        'badge': '29% OFF'
    },
    15: {
        'name': 'Printed Short Kurta',
        'price': 649.00,
        'original_price': 899.00,
        'image': 'shortkurta.jpg',
        'rating': 4.5,
        'reviews': 44,
        'category': 'ethnic',
        'badge': '28% OFF'
    }
}
